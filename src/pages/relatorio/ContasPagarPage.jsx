...
export default function ContasPagarPage() {
  ...
  function toggleSubcat(subcat) {
    setSubcatsAbertas(prev => {
      const novo = new Set(prev)
      novo.has(subcat) ? novo.delete(subcat) : novo.add(subcat)
      return novo
    })
  }
                        ...
                        {open && grupo.map(c => (
                          ...
                          >
                            ...
                            <div style={{ display:'flex', gap:4, flexShrink:0 }}>
                              ...
                              <button onClick={() => abrirEditar(c)} className="btn-action-sm copy" title="Editar">
          ...
          <div className="modal-overlay" onClick={() => { setModalDetalheCat(null); setSubcatsAbertas(new Set()); setBuscaModal('') }}>
            <div className="modal-card modal-contas-cat" onClick={e => e.stopPropagation()}>
              ...
              <div style={{ overflowY:'auto', flex:1, padding:12 }}>
                {subcatsOrd.map(subcat => {
                  const lancamentos = gruposSubcat[subcat]
                  const totalSubcat = lancamentos.reduce((s, c) => s + (Number(c.valor)||0), 0)
                  const aberto = subcatsAbertas.has(subcat)

                  return (
                    <div key={subcat} style={{ marginBottom:8 }}>
                      {/* Cabeçalho da subcategoria (clicável) */}
                      <div
                        ...
                        style={{
                          ...
                          display:'flex', justifyContent:'space-between', alignItems:'center',
                          borderLeft: aberto ? '3px solid var(--blue)' : '3px solid transparent',
                        }}
                      >
                        <div style={{ flex:1 }}>
                          <div style={{ fontSize:14, fontWeight:700, color:'var(--text-header)', marginBottom:2 }}>
                            {subcat}
                          </div>
                        ...
                        <div style={{ display:'flex', alignItems:'center', gap:10 }}>
                          <span style={{ fontSize:15, fontWeight:700, color:'var(--text-body)' }}>
                            {fmtR(totalSubcat)}
                          </span>
                          <svg
                            width="16"
                      ...
                      {/* Lançamentos da subcategoria (expansível) */}
                      {aberto && (
                            ...
                            .map(c => (
                              <div key={c.id}
                                style={{
                                  ...
                                  background:'var(--body-bg)', borderRadius:6,
                                  border:'1px solid var(--border-light)',
                                }}
                              >
                                <div style={{ display:'flex', justifyContent:'space-between', alignItems:'flex-start', marginBottom:8 }}>
                                  <div style={{ flex:1, minWidth:0 }}>
                                    <div style={{ fontSize:13, fontWeight:600, color:'var(--text-header)', marginBottom:3 }}>
                                      {c.observacao || c.categoria}
                                    </div>
                                    <div style={{ fontSize:11, color:'var(--muted)' }}>
                                  ...
                                  <div style={{ textAlign:'right', marginLeft:10 }}>
                                    <div style={{ fontSize:15, fontWeight:700, color: c.status==='PAGO'?'var(--green)':'var(--red)', marginBottom:3 }}>
                                      {fmtR(c.valor)}
                                    </div>
                                    <span style={{
                                ...
                                <div style={{ display:'flex', gap:6, justifyContent:'flex-end' }}>
                                  {c.status !== 'PAGO' && (
                                    <button onClick={() => { setModalPagar({id:c.id}); setDataPag(HOJE); setModalDetalheCat(null); setSubcatsAbertas(new Set()) }}
                                      style={{ ...S.ok, padding:'4px 10px', fontSize:11 }}>
                                      ✓ Pagar
                                    </button>
                                  )}
