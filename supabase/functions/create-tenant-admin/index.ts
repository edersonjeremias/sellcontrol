...
serve(async (req) => {
  ...
  try {
    ...
    // Valida o JWT do usuário chamador usando o admin client (não precisa do anonKey)
    ...
    if (requesterError || !requester || requester.role !== 'master') {
      return new Response(JSON.stringify({
        ok: false,
        message: `Acesso negado. Role encontrada: "${requester?.role ?? 'nenhuma'}". Erro DB: ${requesterError?.message ?? ''}`,
      }), {
        status: 403,
        headers: { ...corsHeaders, 'Content-Type': 'application/json' },
      })
    ...
    // Cria a linha em configuracoes (tabela mestre de tenants)
    ...
    if (configError) {
      throw new Error(`Erro ao criar empresa em configuracoes: ${configError.message}`)
    }
    ...
    if (userError || !userCreated.user) {
      ...
      throw new Error(`Erro ao criar usuário no Auth: ${userError?.message ?? 'sem usuário retornado'}`)
    }

    // Cria o perfil em users_perfil
